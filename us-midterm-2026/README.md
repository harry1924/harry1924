...
- **报告正文**：`report.html`（深度报告，含目录与 24 张图表，浏览器直接打开）
...
- **脚本**：
  ...
  - `scripts/senate_sim.py`：2026 参议院控制权蒙特卡洛测算，输出 `data/senate_sim.json`
...
cd scripts && python3 build_chart_data.py && python3 senate_sim.py && python3 build_report.py
```
